...
//	펀딩 정보
	public FundingDto selectFunding(long funding_id) {
		...
		for (FundingRewardDto fundingRewardDto : rewardList) {
			List<FundingItemDto> itemList = fundingSqlMapper.selectItemList(funding_id,
					fundingRewardDto.getFunding_reward_id());
...
//	공지사항 상세 정보
	public FundingNewsDto selectFundingNews(long funding_notice_id) {
		...
		for (FundingNewsReviewDto fundingNewsReviewDto : reviewList) {
			...
			List<FundingNewsReviewAnswerDto> answerList = fundingSqlMapper.selectNewsAnswerList(funding_notice_id,
					funding_news_review_id);
			fundingNewsReviewDto
					.setAnswer_count(fundingSqlMapper.selectCountAnswer(funding_notice_id, funding_news_review_id));
...
//	펀딩 결제 진행전 주문 정보 조회 -> 리워드별 주문 정보도 같이 조회
	public FundingOrderDto getPaymentBeforeFundingOrder(long funding_order_id) {
		...
		for (FundingRewardOrderDto fundingRewardOrderDto : rewardList) {
			FundingRewardDto fundingRewardDto = fundingSqlMapper
					.selectReward(fundingRewardOrderDto.getFunding_reward_id());

			List<FundingItemDto> itemList = fundingSqlMapper.selectItemList(fundingRewardDto.getFunding_id(),
					fundingRewardDto.getFunding_reward_id());
			fundingRewardDto.setItemList(itemList);
